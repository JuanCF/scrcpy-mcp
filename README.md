...
* **Version Mismatch**: The client must send the exact version of the `scrcpy-server` file. Detection tries `scrcpy --version` on PATH first, then the `scrcpy` binary sitting next to the resolved `scrcpy-server` — so on Windows, or when a GUI app launches the MCP without your shell PATH, pointing `SCRCPY_SERVER_PATH` at a server inside a complete scrcpy install is enough on its own. If neither works (for example a lone `scrcpy-server` downloaded from the releases page, with no `scrcpy` beside it), the client falls back to a default version and the server may exit with a version-mismatch error; set `SCRCPY_SERVER_VERSION` to your installed version (e.g., `"4.1"`). Run the `version` tool to see which version is in use and where it was resolved from. See the Environment Variables table above.
